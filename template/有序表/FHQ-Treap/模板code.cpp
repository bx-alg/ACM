...
int root, id;
int key[N], ls[N], rs[N], cnt[N], siz[N];
double pri[N];
...
void changecnt(int i, int aim, int num)
{
    if (key[i] == aim)
        cnt[i] += num;
    else if (aim < key[i])
        changecnt(ls[i], aim, num);
    else
        changecnt(rs[i], aim, num);
    up(i);
}
...
int small(int i, int x)
{
    if (i == 0)
        return 0;
    if (key[i] < x)
        return siz[ls[i]] + cnt[i] + small(rs[i], x);
    else
        return small(ls[i], x);
}
...
{
    if (siz[ls[i]] < rank && siz[ls[i]] + cnt[i] >= rank)
        return key[i];
    else if (siz[ls[i]] >= rank)
        return index(ls[i], rank);
    else
        return index(rs[i], rank - siz[ls[i]] - cnt[i]);
}
